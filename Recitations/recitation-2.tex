...
Having typical behavior, we can quantify how many (q)bits one has to pay to compress states drawn from given distributions. Recall that we define the fidelity of the compression protocol as the expected fidelity over the drawn state and the mixed state yielded by a decoding-encoding round. 
