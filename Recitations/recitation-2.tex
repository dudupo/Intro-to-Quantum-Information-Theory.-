...
\label{thm:QuantumAEP}
    ...
    \textbf{Direct:}
    for any $n$ large enough,  the typical subspace $T$ with underlying $\epsilon$ satisfies:
    \begin{align}
        \trace\left( \rho^{\otimes n} P_T \right) \geq 1 - \delta.
    \end{align}
    \textbf{Converse:}
    for every subspace $\tT = \tT(n, \varepsilon, \delta)$ in $\cH_d^{\otimes n}$ of dimension $2^{n (S(\rho) - \epsilon)}$, there exists $n$ large enough, such that:
    \begin{align}
