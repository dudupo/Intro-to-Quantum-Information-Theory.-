...
\begin{enumerate}
  \item  Prove that the relation $C_{X} \subset C_{Z}^{\perp}$ implies $H_{Z}H_{X}^\top = 0$, where $H_{Z}$ and $H_{X}$ are the parity check matrices of the codes $C_{X}, C_{Z}$.
    \inblue{ \textbf{[Solution.]} $H_{X}^\top$ is the generator matrix of the subspace spanned by it's colomuns, namly by $H_{X}$ rows, which, by definition, are all the vectors who prepdicular to codewords in $C_X$, Thus, $H_{X}^\top$ is the the generator matrix for the code $C_{X}^\perp$. Since $C_{X}^\perp \subset C_{Z}$ we get the relation $H_{Z}H_{X}^\top = 0 $.  } 
