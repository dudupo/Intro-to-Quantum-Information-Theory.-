...
\begin{enumerate}
  ...
  \item Consider the CSS code $Q(C_{X}, C_{Z})$ and let $f \in \mathbb{F}_{2}^{n}$. Denote by $Z^{f}$ the operator which on the $i$th qubit acts trivially if $f_{i} = 0$ and otherwise applies $Z$. Assume that $|f| < d(C_{Z})/2$. Explain how to correct the noisy state $Z^{f}\ket{C_{Z}^\perp}$ and convince yourself that you can also correct a noisy state of the form $\left( Z^{f_{1}}X^{f_{2}} + Z^{f_{3}} \right) \ket{C_{Z}^\perp}$.
