...
\begin{frame}
  ...
  Let's continue to the quantum case. From now on we will denote by $I(\rho) = \dim \rho - S(\rho)$.
\end{frame}
\begin{frame}
  \frametitle{Quantum no-go.}
...
\uncover<+->{Now, after applying the majority gate, the kets whose third bit is $1$ are kets in $\rho^{\otimes 3}$ which absorb at least two flips.}\uncover<+->{ So, after tracing out the first two qubits, the coefficient of $\ket{1}\bra{1}$ would be less than:
